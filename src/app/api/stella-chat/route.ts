...
function validateRequest(body: unknown): { valid: boolean; error?: string } {
  ...
  if (body.objects.length === 0) {
    return {
      valid: false,
      error: "objects array must contain at least one object",
    }
  }
  ...
  const messages = body.messages as StellaChatMessage[]
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (!msg.role || (msg.role !== "user" && msg.role !== "assistant")) {
      return {
        valid: false,
        error: `messages[${i}].role must be either "user" or "assistant"`,
      }
    }
    if (
      !msg.content ||
