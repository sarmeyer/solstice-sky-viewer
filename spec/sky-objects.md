...
The user enters a location (city, state, country or ZIP). The system returns a
small set of visible celestial objects (stars, planets, prominent objects) for
**tonight**, including basic visibility information.
...
> As a user, I want to enter my location and see a few celestial objects visible
> tonight so I can quickly understand what’s in the night sky without needing
> complex astronomy knowledge.
...
- Each object contains:
  ...
  - short note
- On invalid or missing location, return an error (status 400 or 500) in a
...
- For now, `date` always defaults to **current date (user’s local day)** →
  server may treat as UTC-based “tonight”.
